...
const initialProducts = [
  { id: 1, title: 'Majestic Rose Bouquet', price: 120, image: 'https://images.unsplash.com/photo-1563241527-3004b7be0ffd?w=500&q=80' },
  { id: 2, title: 'Peony Perfection', price: 95, image: 'https://images.unsplash.com/photo-1596431985396-189f7051d7e2?w=500&q=80' },
  { id: 3, title: 'Orchid Elegance', price: 150, image: 'https://images.unsplash.com/photo-1510243452033-0c3dc4dc5062?w=500&q=80' },
  { id: 4, title: 'Lily Radiance', price: 110, image: 'https://images.unsplash.com/photo-1508611130095-2aa08fa88c5f?w=500&q=80' },
  { id: 5, title: 'Tulip Symphony', price: 85, image: 'https://images.unsplash.com/photo-1520763185298-1b434c919102?w=500&q=80' },
];

export default function ProductMarquee({ isAdmin = false }) {
  const [products, setProducts] = useState(() => {
    try {
      const saved = localStorage.getItem('marqueeProducts');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('marqueeProducts', JSON.stringify(products));
    } catch (e) {
      console.warn("Failed to save marquee products to localStorage", e);
    }
  }, [products]);
  ...
  // Create a seamless loop array (dup multiple times for safety on large screens)
  ...
  const handleSaveEdit = () => {
    setProducts(products.map(p => p.id === editingId ? { ...editForm, price: Number(editForm.price) } : p));
    setEditingId(null);
  };
  ...
  const handleAddProduct = () => {
    ...
    const addedProduct = {
      ...
      image: newProduct.image || 'https://images.unsplash.com/photo-1508611130095-2aa08fa88c5f?w=500&q=80' // default
    };
            ...
            <div className="mb-8 p-6 bg-petal-50/50 rounded-xl border border-petal-100 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <input type="text" className="input-field" value={newProduct.title} onChange={(e) => setNewProduct({...newProduct, title: e.target.value})} placeholder="E.g., Radiant Sunflowers" />
                </div>
                <div>
                  <label className="label">Price ($)</label>
                  <input type="number" className="input-field" value={newProduct.price} onChange={(e) => setNewProduct({...newProduct, price: e.target.value})} placeholder="0.00" />
                </div>
                <div>
                  <label className="label">Upload Image</label>
                  <input type="file" accept="image/*" className="input-field py-1.5 text-sm" onChange={(e) => handleImageUpload(e, (url) => setNewProduct({...newProduct, image: url}))} />
                </div>
              </div>
                ...
                {editingId === product.id ? (
                  <div className="space-y-3 animate-fade-in-up">
                    ...
                    <input 
                      ...
                      accept="image/*"
                      className="input-field py-1.5 text-sm" 
                      onChange={(e) => handleImageUpload(e, (url) => setEditForm({...editForm, image: url}))} 
                    />
